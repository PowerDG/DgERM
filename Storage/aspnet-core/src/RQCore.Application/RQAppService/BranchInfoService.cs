...
namespace RQCore.RQAppService
{
    public class BranchInfoService : RQCoreAppServiceBase, IBranchInfoAppService
    {
        ...
        public JsonObject<BranchInfo> CreateMissionQInfo(BranchInfoDto input)
        {
            var task = Mapper.Map<BranchInfo>(input);
            task.Id = null;  //自增列
            ...
            if (task != null)
            {
                ...
                return new JsonObject<BranchInfo>(task);
            }
            else
                return new JsonObject<BranchInfo>();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IList<SelectDto> GetBranchIdList()
        {
