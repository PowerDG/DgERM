...
namespace RQCore.Controllers
{
    ...
    public class TokenAuthController : RQCoreControllerBase
    {
        ...
        [HttpPost]
        public async Task<JsonResult> AuthenticateWithDict([FromBody] AuthenticateModel model)
        {//无用测试
            var loginResult = await GetLoginResultAsync(
                model.UserNameOrEmailAddress,
                model.Password,
            ...
            //int UserRolesCount;// = roles.Count;
            return Json( new   {
                AccessToken = accessToken,
                EncryptedAccessToken = GetEncrpyedAccessToken(accessToken),
                ExpireInSeconds = (int)_configuration.Expiration.TotalSeconds,
                UserId = loginResult.User.Id,
                 UserRolesCount = roles.Count,
                roles.Count
